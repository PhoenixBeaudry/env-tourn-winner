...
def get_reward_funcs(dataset_type: dict, sample_data, has_extra_column: bool):
    ...
    # Check for environment task
    if "environment_name" in dataset_type:
        ...
        try:
            ...
            module_name = f"environment_functions.{real_env_name}"
            env_module = importlib.import_module(module_name)
            
            # Get the reward function
            reward_func_name = f"{real_env_name}_rollout_reward_func"
            # Fallback for generic rollout_reward_func
            if not hasattr(env_module, reward_func_name):
                 reward_func_name = "rollout_reward_func"
                 
            if not hasattr(env_module, reward_func_name):
                 raise ValueError(f"Reward function not found in {module_name}")
            
            reward_func = getattr(env_module, reward_func_name)
            
            # For environment tasks, we wrap the reward function to extract from kwargs
            # Signature must be (prompts, completions, **kwargs) for latest TRL
            def env_reward_wrapper(prompts, completions, **kwargs):
                # Diagnostic: check if env_rewards is present
                if "env_rewards" not in kwargs and int(os.environ.get("LOCAL_RANK", "0")) == 0:
                    print(f"WARNING: 'env_rewards' missing from kwargs in {reward_func_name}. Keys found: {list(kwargs.keys())}")
                
                # Pass both prompts and completions to the underlying handler
                return reward_func(prompts=prompts, completions=completions, **kwargs)
            
            # Assign a name for logging
            env_reward_wrapper.__name__ = f"env_{env_name}_reward"
            
            reward_funcs_callable.append(env_reward_wrapper)
            reward_func_names.append(reward_func_name)
            reward_weights.append(1.0)
            
            print(f"Loaded environment reward function: {reward_func_name}")
            return reward_funcs_callable
            
        except Exception as e:
            raise ValueError(f"Failed to load environment {env_name}: {str(e)}")
            
    else:
        # Standard text/image task configuration
        reward_weights_list = [
        ...
        for i, reward_function in enumerate(dataset_type["reward_functions"]):
            ...
            def create_wrapper(func, w):
                supports_extra = supports_extra_data(func)
                if supports_extra and has_extra_column:
                    return lambda prompts, completions, extra_data, **kwargs: [r * w for r in func(prompts=prompts, completions=completions, extra_data=extra_data, **kwargs)]
                else:
                    return lambda prompts, completions, **kwargs: [r * w for r in func(prompts=prompts, completions=completions, **kwargs)]
            
            reward_funcs_callable.append(create_wrapper(reward_func_callable, reward_weight))
            print(f"Loaded reward function {i} with weight {reward_weight}")
...
def main():
    ...
    if "environment_name" in train_request["dataset_type"]:
        ...
        try:
            ...
            # Use dynamic function name based on environment
            rollout_func_name = f"{real_env_name}_rollout_first_prompt_and_completion"
            # Fallback for generic rollout_first_prompt_and_completion
            if not hasattr(env_module, rollout_func_name):
                 rollout_func_name = "rollout_first_prompt_and_completion"
            
            if hasattr(env_module, rollout_func_name):
                trainer_kwargs["rollout_func"] = getattr(env_module, rollout_func_name)
                print(f"Successfully loaded and attached rollout function: {rollout_func_name}")
            else:
                print(f"WARNING: Rollout function not found in {module_name}")
                
        except Exception as e:
            print(f"ERROR: Failed to load rollout function for {env_name}: {e}")
            raise e
    ...
    # --- Compatibility Fallback: Explicitly attach rollout_func ---
    if "rollout_func" in trainer_kwargs:
        trainer.rollout_func = trainer_kwargs["rollout_func"]
        if is_main_process(LOCAL_RANK):
            print(f"INFO: Manually attached {trainer_kwargs['rollout_func'].__name__} to trainer.")

    # for callback in trainer.callback_handler.callbacks:
    #     if isinstance(callback, AdaptiveGradientCallback):
    ...
    if is_main_process(LOCAL_RANK):
        print(f"DEBUG: Trainer state - use_vllm={training_args.use_vllm}, rollout_func={type(getattr(trainer, 'rollout_func', None))}")
