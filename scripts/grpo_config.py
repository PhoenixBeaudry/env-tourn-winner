...
# 'adamw_torch', 'adamw_torch_fused', 'adamw_torch_xla', 
...
# 'galore_adamw_8bit', 'galore_adafactor', 'galore_adamw_layerwise', 'galore_adamw_8bit_layerwise', 
# 'galore_adafactor_layerwise', 'lomo', 'adalomo', 'grokadamw', 'schedule_free_radam', 
# 'schedule_free_adamw', 'schedule_free_sgd', 'apollo_adamw', 'apollo_adamw_layerwise'

USE_CONFIG = "paged_adamw_8bit"
...
def get_optimizer_info(optimizer_name: str) -> dict:
    if optimizer_name not in OPTIMIZER_CONFIGS:
        print(f"Optimizer {optimizer_name} not found, defaulting to paged_adamw_8bit")
        return OPTIMIZER_CONFIGS["paged_adamw_8bit"]
...
def if_contain_slow_reward_function(dataset_type: dict) -> bool:
    if "reward_functions" not in dataset_type:
        return False
...
def get_training_json(train_info: dict) -> dict:
    ...
    optimizer_type = train_info.get("optimizer_type", USE_CONFIG)
    opt_config = get_optimizer_info(optimizer_type)
    ...
    run_config = {
        ...
        "disable_fa": True if "environment_name" in train_info["dataset_type"] else disable_flash_attention(model_architecture, model_name),
